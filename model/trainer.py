...
class Trainer(object):
    ...
    def train_epoch(self, epoch, loss_weights):
        ...
        for batch_idx, (data, target, idx) in enumerate(self.train_loader):
            ...
            datal, datah = disentangle(data, 'db1', 1)  # DWT 小波变化得到降噪后的信号
            datal = torch.from_numpy(datal).cuda()
            datah = torch.from_numpy(datah).cuda()
            repr1, repr2 = self.model(data, datal, datah, idx)  # nvc
    ...
    def val_epoch(self, epoch, val_dataloader, loss_weights):
        ...
        total_val_loss = 0
        with torch.no_grad():
            for batch_idx, (data, target, idx) in enumerate(val_dataloader):
                datal, datah = disentangle(data, 'db1', 1)  # DWT 小波变化得到降噪后的信号
                datal = torch.from_numpy(datal).cuda()
                datah = torch.from_numpy(datah).cuda()
                repr1, repr2 = self.model(data, datal, datah, idx)  # nvc
    ...
    def train(self):
        ...
        loss_tm1 = loss_t = np.ones(3)  # (1.0, 1.0)
    ...
    @staticmethod
    def test(model, dataloader, scaler, logger, args):
        model.eval()
        y_pred = []
        y_true = []
        # all_representations = []
        with torch.no_grad():
            for batch_idx, (data, target, idx) in enumerate(dataloader):
                datal, datah = disentangle(data, 'db1', 1)  # DWT wavelet transform to obtain the denoised signal
                datal = torch.from_numpy(datal).cuda()
                datah = torch.from_numpy(datah).cuda()
                repr1, repr2 = model(data, datal, datah, idx)  # nvc
        ...
        test_results = []
        if (y_pred.shape[-1] == 2 and y_true.shape[-1] == 2):
            # inflow
